var Paint = (function () {
    ...
    //panel is aligned with the top right
    var PANEL_WIDTH = 300;
    ...
    function Paint (canvas, wgl) {
        ...
        function start(shaderSources) {
            ...
            //units are pixels
            var FIXED_SIZE = 1000;
            var startX = Math.max(INITIAL_PADDING, Math.floor((canvas.width - FIXED_SIZE) / 2));
            var startY = Math.max(INITIAL_PADDING, Math.floor((canvas.height - FIXED_SIZE) / 2));
            this.paintingRectangle = new Rectangle(
                startX, startY,
                Utilities.clamp(FIXED_SIZE, MIN_PAINTING_WIDTH, this.maxPaintingWidth),
                Utilities.clamp(FIXED_SIZE, MIN_PAINTING_WIDTH, this.maxPaintingWidth));
            ...
            this.colorPicker = new ColorPicker(this, 'brushColorHSVA', wgl, canvas, shaderSources, canvas.width - PANEL_WIDTH + COLOR_PICKER_LEFT, 0);
            ...
            this.onResize = function () {
                ...
                this.colorPicker.left = this.canvas.width - PANEL_WIDTH + COLOR_PICKER_LEFT;
    ...
    Paint.prototype.update = function () {
        ...
        var panelLeft = this.canvas.width - PANEL_WIDTH;
        ...
        if (this.needsRedraw) {
            ...
            var blurDrawState = wgl.createDrawState()
                ...
                .viewport(
                    Utilities.clamp(panelLeft - BLUR_FEATHER, 0, this.canvas.width),
                    Utilities.clamp(panelBottom - BLUR_FEATHER, 0, this.canvas.height),
                    PANEL_WIDTH + BLUR_FEATHER * 2,
        ...
        var panelDrawState = wgl.createDrawState()
            .viewport(panelLeft, panelBottom, PANEL_WIDTH, PANEL_HEIGHT)
        ...
        this.drawShadow(PANEL_SHADOW_ALPHA, new Rectangle(panelLeft, panelBottom, PANEL_WIDTH, PANEL_HEIGHT)); //shadow for panel
    ...
    // Returns just the painting area as a data URL (no download)
    Paint.prototype.getDataURL = function () {
        var wgl = this.wgl;
    ...
    Paint.prototype.desiredInteractionMode = function (mouseX, mouseY) { 
        var mouseOverPanel = mouseX > this.canvas.width - PANEL_WIDTH && mouseY > this.canvas.height - PANEL_HEIGHT;
