...
// Physics constants
const FRICTION = 0.88;          // Velocity decay (0-1, lower = more friction)
const WHEEL_SENS = 0.5;         // Mouse wheel sensitivity
const DRAG_SENS = 1.0;          // Drag sensitivity

// Visual constants
const MAX_ROTATION = 22;        // Maximum card rotation in degrees
const MAX_DEPTH = 160;          // Maximum Z-axis depth in pixels
const MIN_SCALE = 0.88;         // Minimum card scale
const SCALE_RANGE = 0.14;       // Scale variation range
const GAP = 50;                 // Gap between cards in pixels
...
function createCards() {
  ...
  IMAGES.forEach((src, i) => {
    ...
    // Mark specific cards as clickable (navigation handled in pointerup)
    const CLICKABLE = ['./g1.png', './g2.png', './g3.png'];
    if (CLICKABLE.includes(src)) {
      card.classList.add('card--clickable');

      // Hover overlay
      const overlay = document.createElement('div');
      overlay.className = 'card__overlay';
      overlay.innerHTML = '<span class="card__overlay-text">▶ Open</span>';
      card.appendChild(overlay);

      // Hover animation via JS (CSS can't due to pointer capture)
...
function updateCarouselTransforms() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // Apply blur to non-adjacent cards, hide far cards
    const isCore = i === closestIdx || i === prevIdx || i === nextIdx;
    if (isCore) {
      it.el.style.filter = 'none';
      it.el.style.opacity = '1';
    } else {
      it.el.style.filter = `blur(3px)`;
...
stage.addEventListener('pointerup', (e) => {
  ...
  const totalMove = Math.abs(e.clientX - pointerStartX) + Math.abs(e.clientY - pointerStartY);

  // If barely moved — treat as a click
  if (totalMove < 8) {
    const clickedCard = pointerTarget && pointerTarget.closest('.card--clickable');
    if (clickedCard) {
      ...
      if (img) {
        ...
        if (CARD_LINKS[src]) {
          // Click animation: shrink → navigate
          ...
          setTimeout(() => {
            window.location.href = CARD_LINKS[src];
          }, 350);
          return;
        }
      }
